...
def _shared_db(monkeypatch):
    ...
    monkeypatch.setattr("api.auth.jwt.SessionLocal", SessionLocal, raising=False)
...
def test_token_verifies_before_revoke():
    from fastapi import HTTPException, status
    token = create_access_token({"sub": "alice"})
    exc = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="x")
    data = verify_token(token, exc)
    assert data.username == "alice"


def test_token_rejected_after_revoke():
    from fastapi import HTTPException, status
    token = create_access_token({"sub": "alice"})
    revoke_token(token)
    exc = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="x")
    with pytest.raises(HTTPException):
        verify_token(token, exc)


def test_revoke_writes_blacklist_row():
    import jwt as _pyjwt
    from api.auth.jwt import get_secret_key
...
def test_revoke_idempotent():
    ...
    token = create_access_token({"sub": "alice"})
    revoke_token(token)
    revoke_token(token)  # no raise


def test_revoke_with_garbage_token_is_safe():
...
def test_revoke_does_not_invalidate_other_tokens():
