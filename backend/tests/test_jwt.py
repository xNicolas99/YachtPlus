...
def mock_secret_key(monkeypatch):
    monkeypatch.setattr("api.auth.jwt.settings.SECRET_KEY", "test_secret_key_1234567890")


def test_get_secret_key(monkeypatch):
    monkeypatch.setattr("api.auth.jwt.settings.SECRET_KEY", "dummy_secret_key_1234")
    assert get_secret_key() == "dummy_secret_key_1234"
...
def test_create_access_token(mock_secret_key):
    ...
    decoded = jwt.decode(token, "test_secret_key_1234567890", algorithms=[ALGORITHM])
