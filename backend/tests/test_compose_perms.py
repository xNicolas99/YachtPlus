...
async def test_compose_start_requires_perm_start(db):
    ...
    with pytest.raises(HTTPException) as exc:
        await get_compose_action(MagicMock(), 
            project_name="demo",
            action="start",
...
async def test_compose_start_succeeds_with_perm_start(db):
    ...
    with patch(
        ...
        result = await get_compose_action(MagicMock(), 
            project_name="demo",
            action="start",
...
async def test_compose_app_action_requires_matching_perm(db):
    ...
    with pytest.raises(HTTPException) as exc:
        await get_compose_app_action(MagicMock(), 
            project_name="demo",
            action="rm",
            app="svc",
            Authorize=MockAuth("starter"),
...
async def test_compose_app_action_passes_through_when_authorised(db):
    ...
    with patch(
        ...
        result = await get_compose_app_action(MagicMock(), 
            project_name="demo",
            action="restart",
            app="svc",
