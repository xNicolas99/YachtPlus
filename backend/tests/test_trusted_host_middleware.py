...
def test_private_ip_host_accepted_when_enabled(monkeypatch):
    app = _build_app({"YACHT_ALLOW_PRIVATE_NETWORK_HOSTS": "true"}, monkeypatch)
    client = _client(app)
    # 192.168.x.y is RFC 1918 — should be allowed when explicitly enabled.
    r = client.get("/api/setup/status", headers={"host": "192.168.1.42:8000"})
    assert r.status_code != 400, r.text


def test_link_local_ip_accepted_when_enabled(monkeypatch):
    app = _build_app({"YACHT_ALLOW_PRIVATE_NETWORK_HOSTS": "true"}, monkeypatch)
    client = _client(app)
...
def test_private_ip_host_rejected_by_default(monkeypatch):
    """With the secure default (ALLOW_PRIVATE_NETWORK_HOSTS=false), private
    IP literals in the Host header are rejected like any other unconfigured
    host.
    """
...
def test_link_local_ip_rejected_by_default(monkeypatch):
    app = _build_app({}, monkeypatch)
    client = _client(app)
    r = client.get("/api/setup/status", headers={"host": "169.254.1.5:8000"})
    assert r.status_code == 400
